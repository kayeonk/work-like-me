...
- **자가 진화(기본 켜짐)** — SessionEnd 훅이 로컬에서 새 신호를 모아 업데이트를 *제안*합니다. `auto` 스킬로 언제든 끌 수 있습니다.
...
/reload-plugins
```
`/reload-plugins`가 없는 빌드면 Claude Code를 재시작하세요. `/` 치면 `/wlm:create`가 보이면 성공.
...
- **자동 배지(Claude만, 기본 켜짐)** — 플러그인 자체 `SessionEnd` 훅이 신호를 로컬에서 조용히 모으다
  충분히 쌓이면 대시보드에 알려줍니다. **settings.json은 안 건드리고**, `auto` 스킬로 끌 수 있습니다(opt-out 플래그).
  Codex엔 이 훅이 없지만 `update`가 양쪽을 스캔합니다.
