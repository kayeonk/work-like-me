...
```
**맨 먼저 ① 언어를 묻는다**(추측 금지). 언어를 정하기 전엔 길게 말하지 말고, **언어 중립적으로**
짧게만 물어라 — 특정 언어를 깔지 말 것. 예: `Select your language:` / 옵션 `한국어` · `English`.
사용자가 고른 언어로 **이후 인터뷰 전체를 진행하고, 규칙·프롬프트·`meta.lang`도 그 언어로** 맞춘다.
(기본값을 제안해도 되지만 최종은 사용자 선택.)

언어를 정한 뒤, 그 언어로 나머지를 묻는다: ② 소스(Claude/Codex/둘 다) ③ 프로젝트(전체/특정)
...
- `meta.lang`: **Step 0에서 사용자가 고른 언어**(`"ko"` 또는 `"en"`)를 그대로 넣는다(추론하지 마라).
...
추가로 목적·설치 대상(Claude/Codex)을 확인해 rules.json에 반영한다. (출력 언어는 Step 0에서 이미 정함.)
