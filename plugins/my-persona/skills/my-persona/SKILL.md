...
이 SKILL.md가 있는 디렉터리를 `${SKILL_DIR}`로 삼아, 아래 명령의 `${SKILL_DIR}`를 실제 경로로 치환해
실행하라. (플러그인 설치면 `<플러그인 루트>/skills/my-persona`, 일반 스킬이면 `~/.claude/skills/my-persona`.)
개인 데이터(rules.json·.pending·.msgs.jsonl)는 위치와 무관하게 항상 **`~/.my-persona/`**에 저장된다.
...
```bash
python3 ${SKILL_DIR}/extract.py --list
```
...
```bash
python3 ${SKILL_DIR}/extract.py --sources claude,codex --projects "<substr>" --since <YYYY-MM-DD>
```
...
```bash
python3 ${SKILL_DIR}/analyze.py    # 패턴 빈도(보조 신호, 언어의존적)
python3 ${SKILL_DIR}/sample.py     # LLM 귀납용 대표 샘플
```
...
```bash
python3 ${SKILL_DIR}/compile.py     # rules.json → 프롬프트 미리보기(stdout)
```
...
```bash
# 미리보기
python3 ${SKILL_DIR}/install.py --from-rules --target ~/.claude/CLAUDE.md
# 승인 후 기록 (Codex는 --target ~/.codex/AGENTS.md)
python3 ${SKILL_DIR}/install.py --from-rules --target ~/.claude/CLAUDE.md --apply
```
...
```bash
python3 ${SKILL_DIR}/dashboard.py   # 브라우저로 규칙 관리 UI
```
...
```bash
python3 ${SKILL_DIR}/capture.py --install-hook
```
...
5. `python3 ${SKILL_DIR}/capture.py --mark-reviewed` (쿨다운 리셋·큐 정리)
